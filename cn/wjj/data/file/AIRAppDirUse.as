	...
	public class AIRAppDirUse
	{
		...
		public static function readFile(path:String):ByteArray
		{
			...
			if(file.exists)
			{
				if(file.isDirectory)
				{
					g.log.pushLog(AIRStorageUse, LogType._UserAction, File.applicationDirectory.nativePath + path + "属于文件夹!");
				}
				else
				{
			...
			{
				g.log.pushLog(AIRStorageUse, LogType._UserAction, File.applicationDirectory.nativePath + path + "文件不存在!");
			}
			return null;
		}
		...
		public static function writeFile(path:String, byte:ByteArray):void
		{
			...
			try{
				...
				g.log.pushLog(AIRAppDirUse,LogType._ErrorLog,"估计是不让写操作!");
			}
